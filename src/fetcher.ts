...
export default class Fetcher {
    ...
    public static async getArticles(cafeid:number):Promise<Article[]> {
        const params:object = {
            "search.clubid":cafeid.toString(),
            "search.boardtype":"L",
            "userDisplay":"5",
        };
        const $:any = await this.getWeb(articleURL,params,true);
    ...
    public static async getComments(cafeid:number,articleid:number):Promise<Comment[]> {
        ...
        const $:any = await this.getWeb(commentURL,params);
